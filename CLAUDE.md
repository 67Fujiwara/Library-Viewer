...
- **読み込んでいない装置も、中の部品名で当たる。** 格納時に書かれる `index.json`（階層一覧）の名前を
  ...
  - **読み込んでいない装置は、以前読み込んだときに付けたタグでも当たる**（`Library.matchedTag` → `Tags.hitUnder('@<保存先>/')`）。
    タグの鍵は保存先のパスなので、閉じてもブラウザに残っている。案件横断は先に対応していたが、
    検索とライブラリタブが見ていなかった（「構成に無い状態だとタグは出ない」の原因はこちら）。
    ライブラリタブのカードには付いているタグをチップで出す (`Library.tagsOf`)
...
- タグのチップを押すとそのタグで検索する
- **検索結果のフィルター** (`07c-filters.js`、結果の見出しの下 `#search-filters`): ピル型の facet（タグ ▾ / 名称 / 種類 ▾ / 装置 / 部署 / 担当者 / 案件コード / 取引先）。
  押すと候補と件数のポップオーバー（バーの幅いっぱい。288px で横にはみ出さない）。同じ facet の中は OR、facet どうしは AND。
  **どの facet を出すかは設定（歯車 → 検索 → 検索結果のフィルター）で切り替える**。既定はタグと名称。
  `Storage` の `lv.searchFilters`。facet を足すときは `FACETS` に 1 行足すだけ
  - **これも「検索を 2 つ持たない」の中に入れる。** フィルターは同じ問い合わせに掛ける追加条件で、
    左のツリー (`Tree.nodeMatches` → `SearchFilters.nodePasses`) と右の一覧 (`SearchFilters.apply`) の両方に効く。
    変えたら `Search.render()` と `Tree.refilter()`（問い合わせはそのまま、行の表示だけ引き直す）
  - 候補の件数は「他の facet で絞った残り」で数える（選んだ facet の候補が消えない）。選んでいる値は 0 件でも残す
  - タグの facet は案件横断と同じ `CrossRef.tagsOf`（自分 + 上の階層のタグ）で数える。ここを別の定義にしない
  - 検索をやめたら (`run('')`) フィルターも `reset()`。次の検索に前の条件を持ち越さない
  - `Esc` は App の 1 か所で: 設定パネル → フィルターのポップオーバー → 計測 → 選択解除 の順
