...
var Store = (function () {
  ...
  function renderRoster() {
    ...
    depts.forEach(function (d) {
      ...
      byDept[d].forEach(function (n) {
        ...
        box.appendChild(el('label.member', { for: id }, [r, n]));
      });
  ...
  async function save() {
    ...
    var preset = App.precision(), pr = Occt.PRESETS[preset];
    ...
    try {
      ...
      devs.forEach(function (d) {
        ...
        metaFiles.push({ name: base, glb: base + '.glb', step: d.stepBytes ? 'step/' + base + '.step' : null, stepSize: d.stepBytes ? d.stepBytes.length : null, glbSize: glb.length, triangles: st.tris, solids: st.solids, rootName: d.name });
        index.devices.push({ file: base, rootName: d.name, tree: d.nodes.map(function (n) { return { name: n.name, path: n.path.join('/'), depth: n.depth, solids: n.solids }; }) });
      });
      var meta = {
        schema: 'library-viewer/1', projectCode: $('#st-project').value.trim(), deviceName: $('#st-device').value.trim(), workpiece: $('#st-work').value.trim(),
        department: selectedOwner.dept, owner: selectedOwner.name, savedAt: isoNowLocal(),
        precision: { preset: preset, linearDeflection: pr.linearDeflection, angularDeflection: pr.angularDeflection },
        files: metaFiles, source: { cad: 'step', app: 'library-viewer' }
      };
