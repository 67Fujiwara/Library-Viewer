...
// 保存先は 1 つに固定。担当者もパスに入る
const owner = (await page.textContent('#st-owner-view')).split('/')[1].trim();
check(preview === `models/設計1課/${owner}/P2026-001_検査装置A/ワークX/`, 'path preview includes the owner: ' + preview);
check(await page.$('#st-layout') === null, 'there is no layout chooser any more');
...
check((await page.textContent('#st-preview')) === 'models/生産技術/高橋/P2026-009_試験機F/ワークZ/', 'path derived from naming rule (owner included)');
