...
var Panels = (function () {
  var KEY = 'lv.panels', state = { left: true, right: true };
  ...
  function init() {
    ...
    apply(false);
    $('#btn-toggle-left').addEventListener('click', function () { toggle('left'); });
    $('#btn-toggle-right').addEventListener('click', function () { toggle('right'); });
    $('#edge-left').addEventListener('click', function () { set('left', true); });
    $('#edge-right').addEventListener('click', function () { set('right', true); });
  }
  // 3D で部品を選んだときなどに勝手に開かないこと。畳んだ状態はユーザーの意思として尊重する
  return { init: init, toggle: toggle, set: set, isOpen: function (s) { return state[s]; } };
})();
