...
右パネルは **検索結果 | 案件横断** の 2 段。検索中だけ結果一覧に入れ替わり、検索をやめると案件横断に戻る。
- ディレクトリにはタグを付けられる（右クリック →「タグを付ける…」）。行にはチップで 2 つまで出し、残りは `+N`
- 検索（`#tree-search`）は **名称とタグ**の両方に当たる。左のツリーは絞り込み、右には
...
- **畳んだ状態は尊重する。** 3D での部品選択やライブラリからの読み込みで勝手に開き直さない
  - 例外は検索。結果の置き場が畳まれていると気づけないので、検索のときだけ開き、
    **検索をやめたら畳み直す**（その間にユーザー自身が開閉したらもう触らない。`Panels.onUserToggle`）
