...
- **大きい STEP はメモリで壊れる。サイズの上限がある。**
  実測 (この環境・標準精度): 1.4MB 3.0 秒 / **19MB 102 秒** / 59MB でタブが落ちた。
  時間はサイズに比例せず**加速して増える**（2.1 秒/MB → 5.4 秒/MB）。
  373MB のファイルはユーザーの PC で `WebAssembly.Exception` になった。
  理由: STEP は読み込むと何倍にもふくらむ（原本 → ワーカーへのコピー → WASM ヒープ → B-rep 数 GB）。
  **この wasm は 32bit なのでアドレス空間が 4GB で頭打ち**。
  - 目安は **50MB 以下なら安心 / 100MB 超は PC 次第**。運用は「ユニット単位で書き出して追加読み込み」
