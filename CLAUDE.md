...
- **大きい STEP は「時間」で詰む。メモリで落ちるのはもっと先。**
  実測 (この環境・標準精度・変換のみ): 1.4MB **3.0 秒** / 19MB **102 秒** / 59MB **764 秒 (12.7 分)**。
  59MB でも 6000 ソリッドを変換しきる。**落ちるのではなく、待てない**のが先に来る。
  1MB あたりの時間が 2.1 → 5.4 → 13 秒/MB と**加速する**ので、外挿してはいけない。
