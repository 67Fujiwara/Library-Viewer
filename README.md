...
**設定は左下の歯車 1 か所**にまとまっています（テーマ / メッシュ精度 / 変換キャッシュ / 名簿 / ネーミングルール / 既定のライブラリ）。

## 使う人（見るだけ）

1. 共有フォルダ（ライブラリ）にある `library-viewer.html` をダブルクリック（Chrome / Edge）
2. 「ライブラリを開く」で共有フォルダを選ぶ → 格納済みの装置が「ライブラリ」タブに並ぶ
   - **選んだ場所は覚えます。** 次からは開いた瞬間に自動で読み込みます
     （ブラウザの権限ダイアログで「毎回このサイトで許可」を選んでおくと完全に自動。
     そうでなければ「接続する」を 1 回押すだけ。自動読み込みは設定で切れます）
...
5. **探す**: 右サイドバーの上の検索欄（`/` か `Ctrl`+`F` で飛べます）に打つと、**名称でもタグでも**当たります
   - フォルダーを右クリック →「タグを付ける…」で、`搬送` `検査` `客先A` のような目印を付けられます
   - ヒットは右パネルに フォルダ → 装置 → 部品 の順で並びます
   - 一覧から選ぶと **そのまとまりだけが残り、関係のない部品はチェックが外れて消えます**
     （「すべて表示」で元に戻ります）。タグのチップを押すとそのタグで検索します
   - タグはこの PC に保存されます（フォルダーのパスが目印。改名・移動しても付いてきます）
6. 落とした STEP は構成ツリー上で整理できます（VS Code のエクスプローラと同じ操作）
...
7. 「この部品に寄る」は、**その部品がいちばん大きく見える角度まで回り込んでから**寄ります
   （板なら板の面が正面に来ます。板の裏に付いた部品なら、板の下へ回り込んで見上げます）
8. 画面から外したい装置は構成ツリーの装置行の × で閉じる。ライブラリから消したいものはカードの「削除」
...
（例: `P2026-001_検査装置A_ワークX_設計1課_山田.step` → `models/設計1課/山田/P2026-001_検査装置A/ワークX/`）。
